...
The final state is only trivial, because we are looking at the radial part of
the wave function of $l = 0$ states (the $S$-orbital)! That is spherically
symmetric.
...
\begin{align*}
    \mathcal M_{fi}\an
    &= \frac{\sqrt 3}{4 \piup \sqrt 2} \frac{m_i \omega_{if}}{c}
    \int_0^\piup \dif \te \, \sin(\te)^2
    \int_0^{2\piup} \dif \phe \,
    \exp(\iup m_i \phe)
    \\&\qquad\cdot
    \sbr{
    ...
    \intertext{%
        The second summand in the square bracket will be integrated away
        because of the periodic exponential function. Only the first summand
        will contribute.
    }
    &= \frac{\sqrt 3}{4 \piup \sqrt 2} \frac{m_i \omega_{if}}{c}
    \int_0^\piup \dif \te \, \sin(\te)^2
    \int_0^{2\piup} \dif \phe \,
    \exp(\iup m_i \phe) \sin(\te) \cos(\tg) \cos(\phe - \phg)
    \intertext{%
        We reorder again.
    }
    &= \frac{\sqrt 3}{4 \piup \sqrt 2} \frac{m_i \omega_{if}}{c}
    \cos(\tg)
    \int_0^\piup \dif \te \, \sin(\te)^3
    \int_0^{2\piup} \dif \phe \,
    \exp(\iup m_i \phe) \cos(\phe - \phg)
    \intertext{%
        From the previous integrals, we already know that the $\te$ integral
        will give $4/3$.
    }
    &= \frac{m_i \omega_{if}}{\piup \sqrt 6 \, c}
    \cos(\tg)
    \int_0^{2\piup} \dif \phe \,
    \exp(\iup m_i \phe) \cos(\phe - \phg)
    \intertext{%
        Now we have enough space to combine the exponential function and the
        cosine.
    }
    &= \frac{m_i \omega_{if}}{2 \piup \sqrt 6 \, c}
    \cos(\tg)
    \int_0^{2\piup} \dif \phe \,
    \sbr{
        \exp\del{\iup\sbr{[m_i + 1] \phe - \phg}}
        + 
        \exp\del{\iup\sbr{[m_i - 1] \phe - \phg}}
    }
    \intertext{%
        We have to look at the cases again. They will only differ by a sign
        that does not make any difference later on in the transition rate.
        Without loss of generality, we will choose $m_i = 1$. The first term
        will again be integrated away due to the periodicity. Only the second
        term will give a contribution. That is constant, so it is just the term
        times $2 \piup$, as in the other cases.
    }
    &= \frac{m_i \omega_{if}}{\sqrt 6 \, c} \cos(\tg) \exp(-\iup\phg)
\end{align*}

From here, we compute the transition rate.
\begin{align*}
    \Gamma_{fi}\an
    &= \frac{\alpha \omega_{if}^3}{12 \piup c^2} \intop_{\Omega_\gammaup} \dif
    \Omega_\gammaup \, \cos(\tg)^2 \\
    \intertext{%
        We insert the coordinates and perform the integration over $\phe$ which
        gives us $2 \piup$.
    }
    &= \frac{\alpha \omega_{if}^3}{6 c^2} \int_0^\piup \dif
    \tg \, \sin(\tg) \cos(\tg)^2
    \intertext{%
        We know this integral already as well, it is $2/3$.
    }
    &= \frac{\alpha}{9 c^2} \omega_{if}^3
\end{align*}
...
\begin{tabular}{c|ccc}
    ...
    $\vec \epsilon_2$ & $\frac{4}{9}$ & $\frac19$ & $\frac19$ \\
\end{tabular}
...
We just need to add the columns in the summary table that we have compiled
earlier:
...
\begin{tabular}{c|ccc}
    ...
    $\vec \epsilon_2$ & $\frac{4}{9}$ & $\frac19$ & $\frac19$ \\
    \midrule
    Total & $\frac49$ & $\frac49$ & $\frac49$
\end{tabular}

One can see that each column has the same total.
...
it the other way around. Since the factors from the angular parts are all the
same, this must be wrong.

Here we collected all the transition rates that we have computed.

\begin{tabular}{lS}
    Transition & {$\Gamma_{fi}\ra / a_0^2$} \\
    \midrule
    $(2P) \to (1S)$ & 1.665 \\
    $(3P) \to (1S)$ & 415.973 \\
    $(3P) \to (2S)$ & 3.358e15
...
This factor does not change anything though, since it is too small or the
transition rates that we have computed are wrong.
